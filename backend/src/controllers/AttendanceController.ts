...
class AttendanceController {
  ...
  static async addAttendanceForClassroom(req, res, next) {
    ...
    const userResponse: any = await SQLHelper.executeQuery(
      SQLHelper.getUserById(userId)
    );
    ...
    const classroomResponse: any = await SQLHelper.executeQuery(
      SQLHelper.getClassroomById(classroomId)
    );
    ...
    const createAttendance = await SQLHelper.executeQuery(
      SQLHelper.createAttendance(
        userId,
        classroomId,
        isPresent,
        attendanceDate,
  ...
  static async editAttendanceForClassroom(req, res, next) {
    ...
    if (isPresent === undefined) {
      return apiResponse(
        "Please provide atleast isPresent attribute",
        RESPONSE.HTTP_BAD_REQUEST,
        {},
        res
      );
    }
    const editAttendance = await SQLHelper.executeQuery(
      SQLHelper.editAttendance(userId, classroomId, attendanceDate, isPresent)
    );
    if (editAttendance === null) {
      return apiResponse(
        "Error in editing attendance",
        RESPONSE.HTTP_BAD_REQUEST,
        {},
        res
      );
  ...
  static async getAttendanceForClassroom(req, res, next) {
    ...
    if (user.userType === USER_TYPES.student) {
      attendanceResponse = await SQLHelper.executeQuery(
        SQLHelper.getAttendanceForClassroomAndUser(classroomId, userId)
      );
    } else {
      attendanceResponse = await SQLHelper.executeQuery(
        SQLHelper.getAttendanceForClassroom(classroomId)
      );
    }
    if (attendanceResponse === null) {
      return apiResponse(
        "Error in fetching attendance",
        RESPONSE.HTTP_BAD_REQUEST,
        {},
        res
      );
    ...
    return apiResponse(
      "Attendance Details for Classroom",
      RESPONSE.HTTP_OK,
      { attendance },
      res
    );
